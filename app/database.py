...
def showItem(role_id, format):
    if format == "all":
        items = (
            session.query(Item)
            .filter(Item.target == role_id, Item.deleted.is_(False))
            .all()
        )
        return items
    elif format == "file":
        items = (
            session.query(Item)
            .filter(
                Item.target == role_id, Item.format == "file", Item.deleted.is_(False)
            )
            .all()
        )
        return items
        ...
        items = (
            session.query(Item)
            .filter(
                Item.target == role_id, Item.format == "plain", Item.deleted.is_(False)
            )
            .all()
        )
        return items
...
def getItemName(id):
    item = session.query(Item).filter(Item.id == id, Item.deleted.is_(False)).first()
    if item:
