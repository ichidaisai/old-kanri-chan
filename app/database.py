...
class Item(Base):
    ...
    __table_args__ = {"mysql_charset": "utf8mb4"}
    id = Column(
        "id", BIGINT(unsigned=True), primary_key=True, unique=True, nullable=False, autoincrement=True
    )
    name = Column("name", VARCHAR(300))
...
def getRole(channel_id):
    role = session.query(Role).filter(Role.chat_tc == channel_id, Role.post_tc == channel_id).first()
    return str(role.id)
